...
export const patchUpdateProfile = async (data: MainProfile, token: string) => {
  const response = await performPatchRequest('/main-profile', data, token)
...
export const buildMainProfilePayload = (data: RegistrationData) => {
  ...
  const buildedData: MainProfile = {
    ...
    accountDetails: { ...accountDetails, dateOfBirth: getJSONDateString(data?.dateOfBirth as string ?? '') ?? '', mobile: data.phone, title: data.title as string },
    proofOfIdentity: {
      ...proofOfIdentity,
